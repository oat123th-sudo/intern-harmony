...
import { Users, UserCheck, TrendingUp, GraduationCap, Loader2 } from "lucide-react";
...
function StatCard({
  ...
  return (
    <Card className="hover-lift group relative overflow-hidden border-border/60 bg-[image:var(--gradient-card)] p-6 shadow-[var(--shadow-soft)] backdrop-blur">
      <div
        className={`absolute -right-10 -top-10 h-32 w-32 rounded-full opacity-50 blur-2xl transition-opacity group-hover:opacity-80 ${ring}`}
      />
      <div className="relative flex items-start justify-between">
        ...
        <div
          className={`flex h-11 w-11 items-center justify-center rounded-xl shadow-sm ${accent}`}
        >
...
function AdminDashboard() {
  ...
  const completion = total
    ? Math.round(((accepted + active) / total) * 100)
    : 0;
      ...
      {usersLoading ? (
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            value={total}
            hint="All-time intern signups"
            icon={Users}
            accent="bg-[image:var(--gradient-primary)] text-primary-foreground"
            ring="bg-primary/20"
          />
          <StatCard
            title="Accepted Interns"
            value={accepted + active}
            hint="Currently or previously accepted"
            icon={UserCheck}
            accent="bg-emerald-500 text-white"
            ring="bg-emerald-300/30"
          />
          <StatCard
            title="Completion Rate"
            value={`${completion}%`}
            hint="Accepted vs applied"
            icon={TrendingUp}
        ...
        <div className="flex items-center justify-between border-b border-border/60 bg-gradient-to-r from-secondary/40 to-transparent p-5">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Monitor tasks, progress, and deadlines for all interns.
            </p>
          </div>
        </div>
        {progressLoading ? (
